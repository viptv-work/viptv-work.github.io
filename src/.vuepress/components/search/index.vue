<template>
  <main id="search-page" class="DocSearch">
    <p v-if="!isAlgoliaConfigured">This search page is not available at the moment, please use the search box in the top navigation bar.</p>
    ...
    <template v-else>
      ...
      <template v-if="results.length">
        <div v-for="(result, i) in results" :key="i" class="search-result">
          ...
          <div class="breadcrumbs"><span v-for="(breadcrumb, j) in result.breadcrumbs" :key="j" class="breadcrumb" v-html="breadcrumb" /></div>
        </div>
      </template>
...
export default {
  ...
  watch: {
    ...
    search() {
      ...
      window.history.pushState({}, 'Vue.js Search', window.location.origin + window.location.pathname + '?q=' + encodeURIComponent(this.search));
    }
  },
  mounted() {
  ...
  methods: {
    ...
    async initializeInfiniteScrollObserver() {
      ...
      this.infiniteScrollObserver = new IntersectionObserver(([{ isIntersecting }]) => {
        if (!isIntersecting || this.totalResults === 0 || this.totalPages === this.lastPage + 1) return;
        this.lastPage++;
    ...
    async updateSearchResults() {
      if (!this.search || !this.algoliaIndex) return;
